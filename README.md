...
- 精簡頂部區塊並加寬工作區；編輯區預設依視窗高度展開，可用滑桿調整兩側高度（300–1400px），或按「自動高度」恢復。
...
右上角可切換主題；編輯器下方的「編輯區高度」滑桿可拖曳，也可用鍵盤方向鍵調整。主題與手動高度會記住至下次開啟，切換設定不會清空 JSON 或復原紀錄。

## 隱私與限制

JSON 原文（包含未完成或格式錯誤的草稿）、主題與編輯區高度會保存於此網站的 localStorage，不會上傳，也不需要伺服器資料庫。紀錄依裝置、瀏覽器及使用者設定檔獨立，不會跨裝置同步。GitHub Pages 仍會接收正常的網站資源請求，但不會收到編輯器內容。
