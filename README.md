...
- 主題支援 Light 淺色、Dark 深色與跟隨系統，包含編輯器語法上色、搜尋面板及比對結果。
...
JSON 只存在目前分頁的記憶體，不會上傳或寫入 localStorage。localStorage 僅保存主題與編輯區高度偏好；若瀏覽器停用儲存，當次仍可調整。GitHub Pages 仍會接收正常的網站資源請求，但不會收到編輯器內容。
